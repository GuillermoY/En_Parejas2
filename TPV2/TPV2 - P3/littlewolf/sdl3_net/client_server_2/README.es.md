...
Este es un ejemplo de arquitectura servidor/cliente donde el servidor recibe mensajes, los imprime en pantalla y responde al cliente con el mensaje "Received".

A diferencia de la versión `client_server_1`, en este caso los clientes no finalizan la ejecución tras el primer envío, sino que solicitan continuamente un mensaje al usuario para enviarlo al servidor.

Existen dos variantes de implementación:

- TCP: El servidor debe gestionar y mantener las conexiones activas con cada cliente.

- UDP: El servidor no mantiene un estado de conexión, ya que es un protocolo orientado a datagramas donde cualquier cliente puede enviar mensajes sin establecer una sesión previa.
