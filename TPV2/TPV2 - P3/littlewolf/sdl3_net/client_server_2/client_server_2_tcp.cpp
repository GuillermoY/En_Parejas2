...
void client(const char *host, Uint16 port) {
	...
	if (NET_WaitUntilInputAvailable(reinterpret_cast<void**>(&conn), 1, 1000)
			> 0) {
		int n = receive(conn, (Uint8*) buffer);
		if (n > 0) {
			buffer[n] = 0;
			...
			std::cout << "Something went wrong" << buffer << std::endl;
		}
	} else {
		done = true;
	...
	while (!done) {
		...
		cout << "Enter a message (type 'done' to exit): ";
		...
		if (!send(conn, (Uint8*)buffer, strlen(buffer)) ){
			error();
		}
	}
