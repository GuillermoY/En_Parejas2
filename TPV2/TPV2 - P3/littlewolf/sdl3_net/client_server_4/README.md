...
This is a server/client example where the server receives several kind of messages corresponding to operation (binary operation, random number, and summation), calculate the result and send it back to the client. The client select which message she wishes to send, fill in the required information, and send it to the server. 

**This version uses structs (that share memory with the buffer) to construct messages.**
