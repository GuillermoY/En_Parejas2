	...
	while (!done) {
		...
		if (NET_WaitUntilInputAvailable(reinterpret_cast<void**>(&conn), 1,
			...
			while (NET_ReceiveDatagram(conn, &packet) && packet != nullptr) {
				...
				switch (packet->buf[0]) { // we know id is the first byte of the Msg struct
				case 0:
					res = handle_binary_operation(packet->buf);
					break;
				case 1:
					res = handle_random_operation(packet->buf);
					break;
				case 2:
					res = handle_sum_operation(packet->buf);
					break;
				default:
					break;
				}
